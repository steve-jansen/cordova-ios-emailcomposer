var assert = require('assert');
var fs     = require('fs');
var path   = require('path');

// we have to stub out the window global for the deprecated function
//var window = {};

// # include the contents of the plugin code
var target = path.join(__dirname, '/../../www/EmailComposer.js');
eval(fs.readFileSync(target)+'');
//target = path.join(__dirname, '/../../www/EmailComposer-Deprecated.js');
//eval(fs.readFileSync(target)+'');

var onSuccessCallback = function (winParam) { 
    console.log('EmailComposer onSuccess - return code ' + winParam.toString());
};

var onErrorCallback = function (error) {
    console.log('EmailComposer onError - ' + error.toString());
};

// mock out cordova's exec method
var cordova = {
  exec: function (successCallback, failCallback, service, action, actionArgs) {
 
    // verify that only functions are passed as callbacks
    if (successCallback !== undefined) {
      assert.equal('function', typeof(successCallback), 'successCallback should be a function or null');
      assert.equal(onSuccessCallback, successCallback, 'wrong callback passed for successCallback');        
    }

    if (failCallback !== undefined) {
      assert.equal('function', typeof(failCallback), 'failCallback should be a function or null');
      assert.equal(onErrorCallback, failCallback, 'wrong callback passed for failCallback');        
    }

    assert.equal('EmailComposer', service, 'the plugin service name should be EmailComposer');

    assert.equal('show', action, 'the action name on the plugin should be show');

    assert(actionArgs instanceof Array, 'the plugin arguments should be an array');

    assert.equal(1, actionArgs.length, 'the plugin arguments should be an array with one element');

    assert.equal('object', typeof(actionArgs[0]),  'the plugin arguments should be an array with one element that is an object');

  }
};


var createParams = function() {
  return { 
          to: 'to@example.com',
          cc: 'cc@example.com',
          bcc: 'bcc@example.com',
          subject: 'Example email message',
          body: '<h1>Hello, world!</h1>',
          isHtml: true,
          attachments: [
            // attach a HTML file using a UTF-8 encoded string
            {
              mimeType: 'text/html',
              encoding: EmailComposer.EncodingType.UTF8,
              data: '<html><body><h1>Hello, World!</h1></body></html>',
              name: 'demo.html'
            },
            // attach a base-64 encoding of http://incubator.apache.org/cordova/images/cordova_128.png
            {
              mimeType: 'text/png',
              encoding: EmailComposer.EncodingType.Base64,
              data: ')',
              name: 'cordova.png'
            }
            // attach a file using a hypothetical file path
            ,{ filePath: './path/to/your-file.jpg' }
          ],
          onSuccess: onSuccessCallback,
          onError: onErrorCallback
        };
}

describe('EmailComposer', function(){
  describe('#show()', function(){
    it('should create a valid cordova actionArgs structure for the iOS plugin', function(){
        var expected = createParams();
        var actual = EmailComposer.show(expected);
        var actualAttachments;

        assert.strictEqual(expected.to, actual.to, 'to line');
        assert.strictEqual(expected.cc, actual.cc, 'cc line');
        assert.strictEqual(expected.bcc, actual.bcc, 'bcc line');
        assert.strictEqual(expected.subject, actual.subject, 'subject line');
        assert.strictEqual(expected.body, actual.body, 'message body');
        assert.strictEqual(expected.isHtml, actual.isHtml, 'message body format');

        // the cordova plugin spec forces us to serialize the actual attachments as JSON
        // so deserialize before checking
        actualAttachments = JSON.parse(actual.attachments);

        assert.strictEqual(expected.attachments.length, actualAttachments.length, '# of email attachments');
        for (var i=0, len=expected.attachments.length; i<len; i++) {
            assert.strictEqual(expected.attachments[i].mimeType, actualAttachments[i].mimeType);
            assert.strictEqual(expected.attachments[i].encoding, actualAttachments[i].encoding);
            assert.strictEqual(expected.attachments[i].data, actualAttachments[i].data);
            assert.strictEqual(expected.attachments[i].filePath, actualAttachments[i].filePath);
        }
    })
  })
})



